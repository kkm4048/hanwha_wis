...
{
    public partial class frm자재입출고관리_입고조회 : Form
    {

        string sql = "";
        public frm자재입출고관리_입고조회()
        {
            InitializeComponent();
        }
        ...
        private void frm자재입출고관리_입고조회_Load(object sender, EventArgs e)
        {
        ...
        private void 조회()
        {
            ...
            nc.DecimalPlaces = 3;
            sql = " exec s_a201_자재_입고_조회 '1','" + cls_com.g공장 + "','" + cmb창고.Text + "','" + cls_com.GetDate(dtp1.Value) + "','" + cls_com.GetDate(dtp2.Value) + "','" + cls_com.GetCode(cmb자재.Text) + "'   "
                        + ",'','','','','','" + txt조회.Text + "'  ";
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                spr.Sheets[0].Cells.Get(0, 13, spr.Sheets[0].RowCount - 1, 14).CellType = nc;
                cls_com.색삭_spr(spr, spr.ActiveSheet.ColumnCount - 1, 8);
            }
